...
import { getCustomerStrategyById, updateCustomerStrategyById } from 'apiSdk/customer-strategies';
...
function CustomerStrategyEditPage() {
  ...
  const handleSubmit = async (values: CustomerStrategyInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCustomerStrategyById(id, values);
      mutate(updated);
      resetForm();
